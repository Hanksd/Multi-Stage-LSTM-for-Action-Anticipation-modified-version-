...
def vgg_action(N_C, input_shape=(224, 224, 3)):
    ...
    for layer in base_model_conv5.layers:
    ...
    x = ZeroPadding2D((1, 1))(x)
    x = AveragePooling2D((16, 16), strides=(16, 16))(x)
    x = Flatten()(x)
...
def MS_LSTM(INPUT_LEN, INPUT_DIM, OUTPUT_LEN, cells=2048):

    input_vs_sp = Input(shape=(INPUT_LEN, INPUT_DIM),name = 'input_1')
    input_vs_tp = Input(shape=(INPUT_LEN, INPUT_DIM),name = 'input_2')
